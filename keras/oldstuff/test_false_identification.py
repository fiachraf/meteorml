...
#        else:
#            false_pred = False
#        pred_list.append((i, pred, false_pred, item))
...
log_file_name = f"{model_name[:-3]}_false_iden_log.csv"
with open(log_file_name, "a") as csv_logfile:
    ...
    for index, pred in enumerate(prediction_list):
        csv_logfile_writer.writerow([get_label(image_paths[index]), round(prediction_list[index][0],2), false_pred(get_label(image_paths[index]), round(prediction_list[index][0],2)), image_paths[index]])
...
fig = plt.hist(prediction_list, bins=100)
plt.title("Prediction_distribution")
plt.xlabel("Prediction")
plt.ylabel("No. of predictions")
plt.savefig(f"{model_name}_pred_dist.png")
